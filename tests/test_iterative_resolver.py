...
def test_standalone_simple_solve():
    eqs, cons, _, _, _ = parse_equations_unified(["od = id + 2*thk"])
...
def test_standalone_supplied_none_with_pinned_equation_inconsistent():
    ...
    """
    eqs, cons, _, _, _ = parse_equations_unified(["x = 5"])
...
def test_comma_eq_broadcast_is_equation():
    eqs, cons, _, _, _ = parse_equations_unified(["x, y = 5"])
    assert len(eqs) == 2
    assert len(cons) == 0


def test_comma_other_op_still_constraint():
    eqs, cons, _, _, _ = parse_equations_unified(["x, y > 0"])
    assert len(eqs) == 0
    assert len(cons) == 2
