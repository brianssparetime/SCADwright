...
def test_orient_top_to_top_flips_peg():
    ...
    plate = cube([40, 40, 2])
    peg = cube([10, 10, 5]).attach(plate, on="top", using_anchor="top", orient=True)
    bb = bbox(peg)
...
def test_orient_bottom_to_bottom_flips_peg():
    ...
    plate = cube([40, 40, 2])
    peg = cube([10, 10, 5]).attach(plate, on="bottom", using_anchor="bottom", orient=True)
    bb = bbox(peg)
...
def test_orient_bottom_to_top_no_rotation_needed():
    ...
    plate = cube([40, 40, 2])
    peg_orient = cube([10, 10, 5]).attach(plate, on="top", using_anchor="bottom", orient=True)
    peg_plain = cube([10, 10, 5]).attach(plate, on="top", using_anchor="bottom")
...
def test_orient_rside_to_bottom():
    ...
    peg = cube([10, 10, 5]).attach(plate, on="rside", using_anchor="bottom", orient=True)
    bb = bbox(peg)
...
def test_orient_preserves_centering():
    ...
    plate = cube([40, 40, 2])
    peg = cube([10, 10, 5]).attach(plate, on="top", using_anchor="bottom", orient=True)
    bb = bbox(peg)
...
def test_orient_opposing_faces_matches_non_orient(on, at):
    """When at and on normals already oppose, orient should match non-orient."""
    plate = cube([40, 40, 2])
    peg_orient = cube([10, 10, 5]).attach(plate, on=on, using_anchor=at, orient=True)
    peg_plain = cube([10, 10, 5]).attach(plate, on=on, using_anchor=at)
    bb_o = bbox(peg_orient)
    bb_p = bbox(peg_plain)
