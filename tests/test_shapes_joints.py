...
def test_alignment_pin_socket_d_default_clearance():
    # default clearance = 0.1
    p = AlignmentPin(d=4, h=8, lead_in=1)
    assert p.clearance == pytest.approx(0.1)
    assert p.socket_d == pytest.approx(4.2)


def test_alignment_pin_socket_d_with_clearance():
...
def test_press_fit_peg_socket_d_smaller_than_shaft():
    ...
    assert p.interference == pytest.approx(0.1)
...
def test_snap_pin_bbox_includes_barbs():
    ...
    p = SnapPin(d=5, h=15, slot_width=1, slot_depth=10, barb_depth=0.8, barb_height=1.5, fn=64)
    bb = bbox(p)
...
def test_snap_pin_socket_sized_for_nominal_plus_clearance():
    p = SnapPin(d=5, h=15, slot_width=1, slot_depth=10, barb_depth=0.8, barb_height=1.5)
    assert p.clearance == pytest.approx(0.2)
