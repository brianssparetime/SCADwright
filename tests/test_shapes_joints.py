...
Joints opt into clearance resolution via ``_clearance_category``; calls
without an explicit ``clearance=`` kwarg inherit from the active scope
or fall through to ``DEFAULT_CLEARANCES``. See ``test_clearances.py``
for the resolution-chain coverage; tests here focus on per-joint
geometry and validation.
"""

import pytest

from scadwright import DEFAULT_CLEARANCES, bbox, emit_str
...
def test_tab_slot_publishes_slot_dims_with_explicit_clearance():
    t = TabSlot(tab_w=5, tab_h=3, tab_d=10, clearance=0.2)
    assert t.slot_w == pytest.approx(5.4)
    assert t.slot_d == pytest.approx(10.4)


def test_tab_slot_uses_default_finger_clearance():
    t = TabSlot(tab_w=5, tab_h=3, tab_d=10)
    expected = DEFAULT_CLEARANCES.finger
    assert t.slot_w == pytest.approx(5 + 2 * expected)
    assert t.slot_h == pytest.approx(3 + expected)
...
def test_alignment_pin_socket_d_with_explicit_clearance():
...
def test_alignment_pin_uses_default_sliding_clearance():
    p = AlignmentPin(d=4, h=8, lead_in=1)
    expected = DEFAULT_CLEARANCES.sliding
    assert p.socket_d == pytest.approx(4 + 2 * expected)


def test_alignment_pin_bbox():
    p = AlignmentPin(d=4, h=8, lead_in=1, fn=64)
    bb = bbox(p)
...
def test_alignment_pin_lead_in_too_large_raises():
    with pytest.raises(ValidationError, match="lead_in"):
        AlignmentPin(d=4, h=1, lead_in=2)


def test_alignment_pin_lead_in_exceeds_radius_raises():
    with pytest.raises(ValidationError, match="lead_in"):
        AlignmentPin(d=4, h=10, lead_in=3)  # 3 > d/2=2
...
def test_press_fit_peg_socket_d_smaller_than_shaft():
    # Press-fit interference: socket is smaller than shaft.
...
def test_press_fit_peg_uses_default_press_clearance():
    p = PressFitPeg(shaft_d=5, shaft_h=6, flange_d=8, flange_h=1.5, lead_in=0.5)
    expected = DEFAULT_CLEARANCES.press
    assert p.socket_d == pytest.approx(5 - 2 * expected)


def test_press_fit_peg_bbox_includes_flange():
    p = PressFitPeg(shaft_d=3, shaft_h=6, flange_d=6, flange_h=1.5, lead_in=0.5, fn=64)
    bb = bbox(p)
...
def test_press_fit_peg_rejects_old_interference_kwarg():
    """After the rename, `interference=` is an unknown kwarg."""
...
def test_snap_pin_bbox_includes_barbs():
    ...
    p = SnapPin(d=5, h=15, slot_width=1, slot_depth=10, barb_depth=0.8,
                barb_height=1.5, fn=64)
    bb = bbox(p)
...
def test_snap_pin_socket_sized_for_nominal_plus_explicit_clearance():
    p = SnapPin(d=5, h=15, slot_width=1, slot_depth=10, barb_depth=0.8,
                barb_height=1.5, clearance=0.2)
    assert p.socket_d == pytest.approx(5.4)


def test_snap_pin_uses_default_snap_clearance():
    p = SnapPin(d=5, h=15, slot_width=1, slot_depth=10, barb_depth=0.8, barb_height=1.5)
    expected = DEFAULT_CLEARANCES.snap
    assert p.socket_d == pytest.approx(5 + 2 * expected)
