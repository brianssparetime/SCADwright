...
composition, ``@variant`` build targets, morph stages) and renders
it as a scadwright project map — a plain-text outline or the same
shape as JSON.
...
- :mod:`.extract` — Param / equations / build / variant / morph extractors.
...
- :mod:`.view` — scadwright-native projection shared by the renderers.
- :mod:`.render_ascii`, :mod:`.render_json` — the two output formats.
"""
