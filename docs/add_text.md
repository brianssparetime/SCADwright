...
**Fonts are named, never paths.** `font` takes a fontconfig family name — the same thing OpenSCAD's `text()` resolves — for example `"DejaVu Sans"` or `"DejaVu Sans:style=Bold"`; `font=None` is OpenSCAD's default Liberation Sans. scadwright resolves that name to a font file with `fc-match` (the same fontconfig OpenSCAD renders through), so one name drives both the render and the metrics. This needs `fc-match` (system fontconfig) in addition to `freetype-py`; if either is missing, or fontconfig can't resolve the name, scadwright warns once and falls back to the `0.6 * font_size` heuristic — OpenSCAD still renders the label via its own fontconfig. If fontconfig substitutes a different family than you asked for, you get a one-time warning. **File paths are rejected:** OpenSCAD's `text()` resolves only by name, so a path can't render; pass the family name instead.
