...
- **Equation-derived edges are missing.** If equations don't validate (the LSP would mark them with squiggles), or if sympy isn't installed (the equations parser needs it), attribute-read edges from those equations are dropped. The rest of the graph still builds. Fix the validation error or `pip install 'scadwright[equations]'` and re-run.
