...
- **Goto-definition** on any name in an equations block, or on a direct ``SourceClass.attr`` reference in Python code: jumps to the `name = Param(...)` assignment for declared Params, or to the equation line that first introduces an auto-declared bare-Name target.
