...
SCADwright bundles a language server with six editor features for `equations = """..."""` blocks. Hover, goto-definition, and rename also work on direct ``SourceClass.attr`` references in Python code outside any equations block. This is the dominant pattern when several files read a shared Spec.
...
- **Hover** on any name in an equations block, or on a direct ``SourceClass.attr`` reference in Python code: curated functions show signatures and brief descriptions; `Param`-declared names show their type, default, and `doc=` text; auto-declared targets show the equation line where they were first introduced.
- **Goto-definition** on any name in an equations block, or on a project-class reference in Python code: jumps to the `name = Param(...)` assignment for declared Params, or to the equation line that first introduces an auto-declared bare-Name target. On a `SourceClass.attr` reference, the class half jumps to the class definition and the attribute half jumps to its equation line. A bare class name, a base in `class Sub(SourceClass)`, an instantiation, and a dotted `pkg.SourceClass` reference all jump to the class.
...
- **Rename refactoring** on any renameable name inside an equations block, or invoked from a direct ``SourceClass.attr`` reference anywhere in the project: updates the `Param` assignment if applicable, every reference inside the same class's equations strings, and references in any other class across the project. Three reference shapes are rewritten: direct ``SourceClass.<old_name>`` (at class scope, inside any method, at module scope, or in a module-level function), ``self.<chain>.<old_name>`` in a method body, and ``<chain>.<old_name>`` in an equations block. The chain can be any depth: ``self.spec.<old_name>`` or a nested ``self.a.b.<old_name>`` both resolve through declared Param types to the class that owns the attribute, so a class that reaches the renamed class only through a chain of Component-typed Params is still updated. Aliased and dotted imports both surface. A reference reached through a local variable (``inner = self.a.b`` then ``inner.<old_name>``) can't be resolved statically and is left unchanged; that reference becomes a missing-attribute error on the next run, since the renamed attribute no longer exists. Single workspace edit; same-file-only when the editor hasn't supplied a workspace folder.
...
Cross-file features read each file's current editor content, so they work against unsaved changes in any open file. The one gap is a brand-new file you have never saved: until it exists on disk the analyzer can't find references in it, so save a new file before relying on a rename to reach across it.
