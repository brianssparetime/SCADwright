...
SCADwright also has a higher-level animation feature with no OpenSCAD equivalent: [morph](morph.md) turns existing variants (e.g. `print` and `display`, or a chain of three or more poses) into an animation that interpolates between them. One line in the Design class, one CLI command, produces an APNG suitable for a README.
...
`relief` is signed: positive raises, negative cuts in. `on=` picks any face by name. On curved walls and rim arcs, glyph layout falls back to a uniform-width heuristic by default since each glyph has to be rotated to follow the surface; install `pip install scadwright[curved-text]` for proportional spacing via freetype-py. See [`add_text()`](add_text.md) for the full reference.
